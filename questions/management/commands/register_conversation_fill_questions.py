...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        level, txt_path, provenance, is_original = resolve_register_io(
            options, 'conversation_questions.txt'
        )
        qs = Question.objects.filter(question_type='conversation_fill', level=level)
        if is_original:
            qs = qs.filter(provenance=provenance)
        qs.delete()
        scope = 'original' if is_original else '全件'
        self.stdout.write(
            self.style.WARNING(f'既存の会話穴埋め問題（level={level}, {scope}）を削除しました')
        )
