...
class CanonicalHostRedirectMiddleware:
    ...
    def __call__(self, request):
        ...
        if host in self.LEGACY_HOSTS and not self._is_exempt(request.path):
            target = f'https://{self.CANONICAL_HOST}{self._canonical_full_path(request)}'
    ...
    def _canonical_full_path(self, request) -> str:
        """Normalize path to trailing-slash form used by Django URL patterns."""
        path = request.path
