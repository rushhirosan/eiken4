...
| original 全件スキャン       | **あり**（`utils/validate_original_questions.py`）                  |
...
[完了] Phase 2: validate_original_questions.py
[いま] Phase 3: 登録→採点→ポイント表示テスト
[次]   Phase 4 …
```
...
- [x] 運用習慣: デプロイ前に `./scripts/release.sh` を必ず通す
...
- [x] 5・4・3 級の original 全ファイルが ERROR 0 で通る（WARN 2 件: 5級語順・Lイラストの正解偏り）
- [x] Phase 1 の preflight から呼ばれる
- [x] `eiken-question-quality-review` スキルに「必ず本スクリプトを実行」と追記済み
...
**最小の次の一歩**: Phase 3 の study_point 表示テストとカテゴリ別 smoke を追加する。

Phase 0–2 完了: `preflight-phase0.sh` / `preflight-original.sh` / `validate_original_questions.py`
