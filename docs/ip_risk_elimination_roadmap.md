...
- [ ] ドメイン切替（現状 `eiken-practice.com` → **`eigogohan.com`**。未取得。取得後に Fly 接続＋旧ドメイン 301）
- [x] フッター常設（市販の定番に合わせる）
...
4. 新ドメイン: **`eigogohan.com`** ← 決定（2026-08-15 時点で未登録＝取得可）。表示ブランド置換は先行済み。切替は取得後

出所の精密分類（公式 vs AI）は**しない**。次は **`eigogohan.com` の取得**（任意だがブランド仕上げ）と5級の新規自作（Phase 3）。
