...
class Command(BaseCommand):
    help = 'Create grammar fill questions from text file'

    def handle(self, *args, **options):
        # 既存の問題を削除
        Question.objects.filter(question_type='grammar_fill').delete()
        self.stdout.write('Deleted existing grammar fill questions')

        # テキストファイルを読み込む
        with open('questions/grammar_fill_questions.txt', 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        self.stdout.write(self.style.SUCCESS('Successfully created all grammar fill questions')) 