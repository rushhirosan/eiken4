...
class Command(BaseCommand):
    help = 'Creates a sample listening question for Eiken Grade 4'

    def handle(self, *args, **options):
        try:
            # ファイルパスを修正
            file_path = os.path.join(settings.BASE_DIR, 'questions', 'listening1_questions.txt')
            ...
            for block_num, block in enumerate(blocks):
                ...
                choices_text = '\n'.join(choices)
                question_text = f"""[音声を聞いて、以下の質問に答えなさい]\n\n{choices_text}\n\nQuestion: What is the correct response?"""
                try:
                ...
                audio_path = os.path.join(settings.MEDIA_ROOT, 'audio', f'eiken4_listening{question_number}.mp3')
                if os.path.exists(audio_path):
                    question.audio_file = f'audio/eiken4_listening{question_number}.mp3'
                    question.save()
                self.stdout.write(self.style.SUCCESS(f'Successfully created listening question {question_number}'))
